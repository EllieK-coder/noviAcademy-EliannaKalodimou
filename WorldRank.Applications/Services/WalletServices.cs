...
public class WalletService
{
    ...
    public void AddWalletToPlayer()
    {
        ...
        Currency? currency = Prompts.PromptCurrency();
        if (currency is null)
            return;
    ...
    public void GetWalletsOfPlayer()
    {
        ...
        List<Wallet> wallets = _walletRepository.GetAllWalletsByPlayerId(playerId.Value);
    ...
    public void ApplyFundsStrategy()
    {
        ...
        FundsOperation? operation = Prompts.PromptFundsOperation();
        ...
        IFundsStrategy strategy = _fundsStrategies[operation.Value];

        RunWalletOperation(() =>
        {
            Wallet wallet = _walletRepository.GetWallet(playerId.Value, currency.Value);
            strategy.Execute(wallet, amount.Value);

            // Persist the strategy's change so it survives with the DB repository.
            _walletRepository.UpdateBalance(playerId.Value, currency.Value, wallet.Balance);
    ...
    private void RunWalletOperation(Action operation)
    {
        ...
        {
            _logger.LogError(ex, "Unexpected error while running wallet operation");
